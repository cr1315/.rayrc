...
__rayrc_install_vim() {
    ...
    if [[ ! -d "${__rayrc_dir_data_git}/__rayrc_backup" ]]; then
        mkdir -p "${__rayrc_dir_data_git}/__rayrc_backup"
    fi


    # backup the user's .vimrc or even .vim folder
    if [[ -f "$HOME/.vimrc" ]]; then
        mv "$HOME/.vimrc" "${__rayrc_dir_data_git}/__rayrc_backup/.vimrc"
    fi
    if [[ -d "$HOME/.vim" && ! -h "$HOME/.vim" ]]; then
        mv "$HOME/.vim" "${__rayrc_dir_data_git}/__rayrc_backup/.vim"
    fi

    ### download plug.vim
    # __rayrc_url_downloader https://raw.githubusercontent.com/junegunn/vim-plug/master/plug.vim \
    #    "${__rayrc_dir_data_git}/vimfiles/autoload/plug.vim"
    curl -fLo "${__rayrc_dir_data_git}/vimfiles/autoload/plug.vim" --create-dirs \
        https://raw.githubusercontent.com/junegunn/vim-plug/master/plug.vim

    ### ln our vimfiles to ~/.vim
    ln -snf "${__rayrc_dir_data_git}/vimfiles" ~/.vim

    vim -u "${__rayrc_dir_data_git}/vimfiles/plugins.vim" +PlugInstall +qa >& /dev/null
    echo "###### after PlugInstall #####"
    pwd
    ls -ahl "${__rayrc_dir_data_git}/vimfiles"
    ls -ahl "${__rayrc_dir_data_git}/vimfiles/plugged"
