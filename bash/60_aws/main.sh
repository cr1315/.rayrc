...
aws.mfa() {
    ...
    credential=$(aws sts get-session-token --duration-seconds $duration --serial-number $serial_number --token-code $authCode)
...
aws.stg() {
    ...
    # targetRole=$(aws iam list-roles | jq -r '.Roles[].Arn' | grep 'From')
...
aws.iac_dev() {
    ...
    if [[ $? != 0 ]]; then
        ...
        if [[ $# -gt 0 && $1 =~ [0-9]{6} ]]; then
            ...
            read -p "need to mfa yourself first, code please: " authCode
        fi
        aws.mfa $authCode 900
        rc=$?
        if [[ $rc != 0 ]]; then
