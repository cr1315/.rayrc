...
__rayrc_setup() {
    ...
    # echo "\$__rayrc_setup_dir: $__rayrc_setup_dir"
    ...
    for dir in `ls -1 "${__rayrc_setup_dir}"`; do
        # echo "\$__rayrc_setup_dir/\$dir: $__rayrc_setup_dir/$dir"
        if [[ -d "$__rayrc_setup_dir/$dir" && -f "$__rayrc_setup_dir/$dir/main.zsh" && ! -f "$__rayrc_setup_dir/$dir/disabled" ]]; then
